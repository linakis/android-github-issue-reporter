 ...
 */
class GistService(private val tokenStorage: SecureTokenStorage) {
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                if (!timberLogs.isNullOrBlank()) {
                    files["timber_logs.txt"] =
                        GistFile(content = buildFileHeader("Timber Logs", timestamp) + timberLogs)
                }

                // Add OkHttp logs if present
                if (!okHttpLogs.isNullOrBlank()) {
                    files["network_logs.txt"] =
                        GistFile(
                            content =
                                buildFileHeader("Network Logs (OkHttp)", timestamp) + okHttpLogs
                        )
                }
                ...
                if (!logcatLogs.isNullOrBlank()) {
                    files["logcat.txt"] =
                        GistFile(content = buildFileHeader("Logcat Output", timestamp) + logcatLogs)
                }

                // Add screenshot if present (as HTML file with embedded image)
                if (!screenshotBase64.isNullOrBlank()) {
                    files["screenshot.html"] =
                        GistFile(content = buildScreenshotHtml(screenshotBase64, timestamp))
                }
                ...
                val request =
                    CreateGistRequest(
                        description =
                            "Logs for: $issueTitle - Generated by GHReporter at $timestamp",
    ...
    companion object {
        @Volatile private var instance: GistService? = null

        /** Get singleton instance. */
        fun getInstance(tokenStorage: SecureTokenStorage): GistService {
            return instance
                ?: synchronized(this) {
                    instance ?: GistService(tokenStorage).also { instance = it }
                }
        }
    }
}

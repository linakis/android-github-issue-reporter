...
object ImageUtils {
    ...
    suspend fun processImage(
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                val resizedBitmap =
                    if (sampledBitmap.width > maxWidth) {
                        val ratio = maxWidth.toFloat() / sampledBitmap.width
                        val newHeight = (sampledBitmap.height * ratio).toInt()
                        val scaled =
                            Bitmap.createScaledBitmap(sampledBitmap, maxWidth, newHeight, true)
    ...
    /**
     * Create a Data URI from Base64 encoded image. This can be used in markdown:
    ...
    /**
     * Create markdown image tag with embedded Base64. Note: This may not render in all markdown
     * viewers.
     */

 ...
 *
 * Only captures logs from the current process ID for privacy. Runs on IO dispatcher to avoid
 * blocking the main thread.
 */
class LogcatCollector(private val maxLines: Int = 500) {
    ...
    suspend fun collect(): String =
        withContext(Dispatchers.IO) {
            try {
                ...
                val process =
                    Runtime.getRuntime().exec(arrayOf("logcat", "-d", "-v", "time", "--pid=$pid"))
     ...
     */
    suspend fun collectWithLevel(level: String): String =
        withContext(Dispatchers.IO) {
            try {
                val pid = Process.myPid()
                ...
                val process =
                    Runtime.getRuntime()
                        .exec(arrayOf("logcat", "-d", "-v", "time", "--pid=$pid", "*:$level"))
     ...
     * @return Filtered logcat output
     */
    suspend fun collectByTag(tag: String): String =
        withContext(Dispatchers.IO) {
            try {
                val pid = Process.myPid()
                ...
                val process =
                    Runtime.getRuntime()
                        .exec(arrayOf("logcat", "-d", "-v", "time", "--pid=$pid", "-s", tag))
    ...
    /**
     * Clear the logcat buffer for this process. Note: This requires elevated permissions and may
